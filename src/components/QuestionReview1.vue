<template>
  ...
  >
    <template v-slot:activator="{on,attrs}">
      ...
      <v-row class="header-row mt-3" justify="center">
      ...
      >
        ...
        >
        <span class="number-text important-font ">{{i+1}}</span>
      </div>
        <!--
        <v-icon
        color="primary"
    ...
    <v-row justify="end">
      ...
      <v-row no-gutters v-if="currentQuestion['answeredCorrectly'].index === -2" class="mb-2" justify="center" align="end">
        <v-col class=" col-auto pa-0 text-h6 d-flex align-items-baseline">
          <v-icon
      ...
      class="col-9 rounded-pill dialog-answer-choice"
      @click="hideDescription(i)">
        <div class="d-flex">
        <v-icon class="mr-3 choice-letter" :color="getIconColor(i)"> mdi-alpha-{{choiceIndexMap[i]}}-circle</v-icon>
        <span class="answer-choice-text my-auto font-weight-bold">{{choice.choice_text}}</span>
      </div>
      </v-col>
